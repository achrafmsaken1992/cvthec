...
export class ManagersService {
    ...
    getManagers(nom: string, prenom: string, nomEntreprise: string, active: number, valide: number, page: number, size: number) {
        ...
        return this.http.get<any>("http://localhost:8080/getManagers?nom=" + nom + "&prenom=" + prenom + "&nomEntreprise=" + nomEntreprise + "&active=" + active + "&valide=" + valide + "&size=" + size + "&page=" + page, {headers: new HttpHeaders({'Authorization': this.jwtToken})});
